...
class Ant(Insect):
    ...
    def add_to(self, place):
        assert place.ant is None or place.ant.can_contain(self) or self.can_contain(place.ant), 'Two ants in {0}'.format(place)
        if place.ant is None:
            place.ant = self
        if place.ant.can_contain(self):
            if place.ant.contained_ant == None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        current_place = ant_place
        counter = 0
        """用self.name或者自己添加一个type varible来确定哪个蚂蚁哪种情况"""
        while current_place is not beehive and self.type == 0:   #min_range == 0 and max_range == float('inf'):
            if current_place.bees == []:
                current_place = current_place.entrance
                counter = counter + 1
            if (current_place.bees and counter > self.max_range) or (current_place.bees and counter < self.min_range):
                return None
            if current_place.bees and counter <= self.max_range and counter >= self.min_range:
                if len(current_place.bees) > 1:
                    return rANTdom_else_none(current_place.bees)
                if len(current_place.bees) == 1:
                    return current_place.bees[0]
            if current_place.entrance is beehive:
                 return None
        while current_place is not beehive and self.type == 1:    #min_range == ShortThrower.min_range and max_range == ShortThrower.max_range:
            if current_place.bees == []:
                current_place = current_place.entrance
                counter = counter + 1
            ...
            if current_place.entrance is beehive:
                return None 
        while current_place is not beehive and self.type == 2:   #min_range == LongThrower.min_range and max_range == LongThrower.max_range:
            if counter < self.min_range:
                current_place = current_place.entrance
                counter = counter + 1                
            if counter > self.min_range - 1:
                if current_place.bees:
                    if len(current_place.bees) > 1:
...
class ShortThrower(ThrowerAnt):
    ...
    #ThrowerAnt.nearest_bee(self, beehive, long_t = False, short_t = True)
...
class LongThrower(ThrowerAnt):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 4
    #ThrowerAnt.nearest_bee(self, beehive, long_t = True, short_t = False)
    implemented = True   # Change to True to view in the GUI
    # END Problem 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor > amount:            
            Ant.reduce_armor(self, amount)   #注意继承前面的class method 不是self.armor = Ant.reduce_armor(self, amount)， 因为Insect class的reduce armor是没有return value
            for bees in current_place.bees[:]:
...
####################################################
        ...
        # if current_place.ant == None:
        #     for bees in current_place.bees[:]:
        #         bees.armor = bees.armor - 3 #Insect.reduce_armor(bees, 3)
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        its armor to 0."""
        # BEGIN Problem 8
        #先要把insect加到place，再reduce armor
        "*** YOUR CODE HERE ***"
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        self.identity = True
        QueenAnt.identity_counter = QueenAnt.identity_counter + 1
        if QueenAnt.identity_counter > 1:
            self.identity = False
    ...
    def action(self, gamestate):
        ...
        if self.identity == True:
            ThrowerAnt.throw_at(self, ThrowerAnt.nearest_bee(self, gamestate.beehive))        #调用其他class的method时必须要写self或者其他object，如果调用自己的method则不用    
        while self.identity == True and current_place.exit != None:
            if current_place.ant == None:
                current_place = current_place.exit
            else:
                if current_place.ant.double == False:
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.identity == True:
            bees_win()
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        # if isinstance(other, ContainerAnt) == True:
        #     if other.contained_ant == None:
        #         return True 
        #     else:
        #         return False
        if isinstance(self, ContainerAnt):
            if self.contained_ant == None:
                if isinstance(other, ContainerAnt) == False:
                    return True
        # if isinstance(other, ContainerAnt) == False:
        #     return True
        else:
    ...
    def action(self, gamestate):
        # BEGIN Optional 2
        "*** YOUR CODE HERE ***"
        if isinstance(self.contained_ant, Ant):
...
def make_slow(action, bee):
    ...
    if gamestate.time % 2 == 0:
        return bee.action(gamestate)
    else:
        bee.place = bee.place.entrance
        return bee.action(gamestate)
    # END Problem Optional 4s
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while current_place is not beehive:
            for bee in current_place.bees:
                my_dict[bee] = distance
            if current_place.ant:
                if distance != 0:
                    my_dict[current_place.ant] = distance
            current_place = current_place.entrance
            distance = distance + 1
        return my_dict
        # END Problem Optional 5
    ...
    def calculate_damage(self, distance):
        ...
        if self.real_damage > 0:
            self.real_damage = self.damage - distance * 0.2 - self.insects_shot * 0.05 
        else:
            self.real_damage = 0
